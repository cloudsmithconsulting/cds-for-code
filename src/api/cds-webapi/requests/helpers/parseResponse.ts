 ...
 * @returns {any} parsed batch response
 */
function parseBatchResponse(response: string, parseParams?: any[], requestNumber?:number) {
        ...
        if (batchResponse.indexOf('--changesetresponse_') > -1) {
            ...
            if (!responseData) {
                if (/Content-Type: text\/plain/i.test(batchResponse)) {
                    ...
                    result.push(isNaN(<number><unknown>plainContent) ? plainContent : parseInt(plainContent));
                }
                else
 ...
 */
export default function parseResponse(response: string, responseHeaders: any, parseParams?: any[]): any {
    let parseResult;
