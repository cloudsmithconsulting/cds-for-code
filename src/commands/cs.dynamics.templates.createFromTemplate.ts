...
export default async function run(destinationUri?: vscode.Uri, type?:TemplateType, template?:TemplateItem): Promise<void> {
    ...
    if (!path) {
        Quickly.error("You must select a workspace and folder before you can create a templated project or item", false, "Try Again", () => { vscode.commands.executeCommand(cs.dynamics.templates.createFromTemplate, destinationUri, type); }, "Cancel");
